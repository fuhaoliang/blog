...
export default function({ isClient, app, $axios, store }) {
  const tipMsg = {
    '401': '请重新登录',
    '403': '未授权'
  }
  ...
  for (const i in services) {
    ...
    for (const ind in service) {
      ...
      Http[i][ind] = async function(
        ...
        let data = newParams
        if (Array.isArray(params) || (isClient && params instanceof FormData)) {
          data = params
        }
        const config = {
          headers: {},
        ...
        if (
          ...
          response = await $axios[api.method](host + apiUrl, data, config)
        } else {
          config.params = newParams
          response = await $axios[api.method](host + apiUrl, config)
        }
        const errorObj = {
          status: {
            code: -1,
        ...
        if (response.response) {
          ...
          errorObj.status.message =
            ...
            (response.response.statusText
              ? response.response.statusText
              : tipMsg[response.response.status])
